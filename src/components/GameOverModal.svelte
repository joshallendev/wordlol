...
<div in:fade out:fade class="fixed flex items-center flex-col inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="flex flex-col mt-3 text-center bg-white w-3/4 md:w-1/2 pt-2 pb-6 rounded mt-20">
        ...
        <div class="mt-2 text-center">
            ...
            <p class="py-2">You win <strong>{newStats.winPct}%</strong> of the time</p>
            {#if $hasWon }
                <p class="py-2">Beat today's WORDLOL in <strong>{newStats.numGuesses}</strong> {newStats.numGuesses > 1 ? 'guesses' : 'guess'}</p>
            {/if}
            <button type="button" class="bg-actionred p-2 mt-4 rounded align-middle" on:click="{handleShare}"> 
